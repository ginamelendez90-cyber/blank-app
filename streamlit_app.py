...
def procesar_datos(texto, deporte):
    ...
    for bloque in bloques:
        ...
        if matches:
            ...
            # BTTS: Ambos marcan (marcador local > 0 Y marcador visitante > 0)
...
def analizar_contexto(texto):
    ...
    t = texto.lower()
    if any(w in t for w in ["menos de", "pocas anotaciones", "under", "baja produccion"]):
        ajuste["goles"] -= 10
        ajuste["notas"].append("📉 Contexto: Tendencia UNDER detectada.")
    if any(w in t for w in ["más de", "over", "ambos marcan", "goleador"]):
        ajuste["goles"] += 10
...
with tab1:
    st.header("Tenis")
    input_tenis = st.text_area("Datos Tenis:", height=150, key="texto_tenis")
...
with tab2:
    st.header("Fútbol: Valor Completo")
    data_f = st.text_area("1. Datos 365Scores:", height=150, key="texto_futbol")
    contexto_f = st.text_area("2. Contexto/Noticias:", height=80)
    
    st.subheader("💰 Cuotas de la Casa")
    col1, col2, col3 = st.columns(3)
    with col1:
        c_loc = st.number_input("Cuota Local", value=2.0)
        c_vis = st.number_input("Cuota Visita", value=3.0)
    with col2:
        c_o15 = st.number_input("Cuota Over 1.5", value=1.3)
        c_u15 = st.number_input("Cuota Under 1.5", value=3.5)
    with col3:
        c_o25 = st.number_input("Cuota Over 2.5", value=2.0)
        c_u25 = st.number_input("Cuota Under 2.5", value=1.8)
    
    c_btts = st.number_input("Cuota BTTS (Ambos Marcan)", value=1.9)
